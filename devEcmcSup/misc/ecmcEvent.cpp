...
int ecmcEvent::setDataSampleTime(int sampleTime) {
  if (sampleTime <= 0) {
    return setErrorID(__FILE__,
                      __FUNCTION__,
                      __LINE__,
                      ERROR_EVENT_INVALID_SAMPLE_TIME);
  }
...
int ecmcEvent::execute(int masterOK) {
  if (!masterOK) {
    return setErrorID(__FILE__,
                      __FUNCTION__,
                      __LINE__,
                      ERROR_EVENT_HARDWARE_STATUS_NOT_OK);
  }
  ...
  if (eventType_ == ECMC_EDGE_TRIGGERED) {
    if (validateEntry(0)) {  // Trigger entry
      return setErrorID(__FILE__,
                        __FUNCTION__,
                        __LINE__,
                        ERROR_EVENT_TRIGGER_ECENTRY_NULL);
    }
  }
...
void ecmcEvent::printStatus() {
  LOGINFO10(
    "%s/%s:%d: INFO: Event %d. Armed: %d, enableArmSequence_: %d, Error: 0x%x.\n",
    __FILE__,
    __FUNCTION__,
    __LINE__,
    index_,
...
int ecmcEvent::linkEventConsumer(ecmcEventConsumer *consumer, int index) {
  if ((index < 0) || (index >= ECMC_MAX_EVENT_CONSUMERS)) {
    return setErrorID(__FILE__,
                      __FUNCTION__,
                      __LINE__,
                      ERROR_EVENT_CONSUMER_INDEX_OUT_OF_RANGE);
  }
