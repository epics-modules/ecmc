...
*
* Credits: From the beginning this state machine code was executed in ecmc-PLC code and was mainly developed by Alvin Acerbo at PSI.
*
...
* slaved axes are disabled and the state is set to ECMC_MST_SLV_STATE_SLVS_IN_CHRGE. If a axis in the master axes group gets a command then the state
...
enum masterSlaveStates {
  ...
  ECMC_MST_SLV_STATE_SLVS_IN_CHRGE = 1,
  ECMC_MST_SLV_STATE_MSTS_IN_CHRGE = 2,
  ECMC_MST_SLV_STATE_ERROR         = 3,
};

class ecmcMasterSlaveStateMachine : public ecmcError {
  public:
    ecmcMasterSlaveStateMachine(int index,
                                const char *name,
