...
int ecmcMotorRecordAxis::applyMotorSoftLimitChange(bool   updateFwd,
                                                   double value) {
  double currentBwd = 0.0;
  double currentFwd = 0.0;
  double oldBwd     = 0.0;
  double oldFwd     = 0.0;
  int    errorCode  = 0;
  ...
  oldBwd = drvlocal.ecmcAxis->getMon()->getSoftLimitBwd();
  oldFwd = drvlocal.ecmcAxis->getMon()->getSoftLimitFwd();
  ...
  if (!errorCode) {
    currentBwd = drvlocal.ecmcAxis->getMon()->getSoftLimitBwd();
    currentFwd = drvlocal.ecmcAxis->getMon()->getSoftLimitFwd();

    if (currentBwd == 0.0 && currentFwd == 0.0) {
      errorCode = drvlocal.ecmcAxis->getMon()->setEnableSoftLimitBwd(0);
      if (!errorCode) {
        errorCode = drvlocal.ecmcAxis->getMon()->setEnableSoftLimitFwd(0);
      }
    } else if (oldBwd == 0.0 && oldFwd == 0.0 &&
               currentBwd < currentFwd) {
      errorCode = drvlocal.ecmcAxis->getMon()->setEnableSoftLimitBwd(1);
      if (!errorCode) {
        errorCode = drvlocal.ecmcAxis->getMon()->setEnableSoftLimitFwd(1);
      }
    }
  }
...
bool ecmcMotorRecordAxis::softLimitValueMatches(bool   updateFwd,
                                                double value) {
  double current = 0.0;

  if (ecmcRTMutex) epicsMutexLock(ecmcRTMutex);
  current = updateFwd ? drvlocal.ecmcAxis->getMon()->getSoftLimitFwd()
                      : drvlocal.ecmcAxis->getMon()->getSoftLimitBwd();
  if (ecmcRTMutex) epicsMutexUnlock(ecmcRTMutex);

  return ecmcMotorRecordSoftLimitEqual(current, value);
}

bool ecmcMotorRecordAxis::softLimitEnableMatches(bool updateFwd,
                                                 int  value) {
  bool current = false;

  if (ecmcRTMutex) epicsMutexLock(ecmcRTMutex);
  current = updateFwd ? drvlocal.ecmcAxis->getMon()->getEnableSoftLimitFwd()
                      : drvlocal.ecmcAxis->getMon()->getEnableSoftLimitBwd();
  if (ecmcRTMutex) epicsMutexUnlock(ecmcRTMutex);
...
asynStatus ecmcMotorRecordAxis::readBackAllConfig(int axisID) {
  ...
  if (status == asynSuccess) status = syncSoftLimitInterfaces(true);
...
asynStatus ecmcMotorRecordAxis::setEnable(int on) {
  ...
  if (drvlocal.ecmcAxis->getBlockCom() && on) {
    if (ecmcRTMutex) epicsMutexUnlock(ecmcRTMutex);
    drvlocal.ecmcAxis->setExternalCommandBlockedError();
    LOGERR(
...
asynStatus ecmcMotorRecordAxis::setIntegerParam(int function, int value) {
  ...
  if (function == pC_->motorUpdateStatus_) {
    ...
    if (softLimitEnableMatches(true, value)) {
      return asynMotorAxis::setIntegerParam(function, value);
    }
    ...
    if (softLimitEnableMatches(false, value)) {
      return asynMotorAxis::setIntegerParam(function, value);
    }
...
asynStatus ecmcMotorRecordAxis::setDoubleParam(int function, double value) {
  ...
  if (function == pC_->motorMoveRel_) {
    ...
    if (softLimitValueMatches(true, value)) {
      return asynMotorAxis::setDoubleParam(function, value);
    }
    ...
    errorCode = applyMotorSoftLimitChange(false, value);
    asynMotorAxis::setDoubleParam(function, value);
    return errorCode == 0 ? asynSuccess : asynError;
...
asynStatus ecmcMotorRecordAxis::setHighLimit(double highLimit) {
  int errorCode = 0;
  const bool softLimitChanged = !softLimitValueMatches(true, highLimit);

  if (isSoftLimitSyncEnabled() && softLimitChanged) {
    errorCode = applyMotorSoftLimitChange(true, highLimit);
...
asynStatus ecmcMotorRecordAxis::setLowLimit(double lowLimit) {
  int errorCode = 0;
  const bool softLimitChanged = !softLimitValueMatches(false, lowLimit);
