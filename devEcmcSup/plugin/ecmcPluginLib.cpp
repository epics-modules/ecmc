...
//                                       "_plugin_get_data")) == NULL) {
  ...
  if (data_->ifVersion != ECMC_PLUG_VERSION_MAGIC) {
    LOGERR(
      "%s/%s:%d: Error: Plugin %s: Interface version missmatch (%d!=%d) (0x%x).\n",
      __FILE__,
      __FUNCTION__,
      __LINE__,
      libFilenameWP,
      ...
      data_->ifVersion,
      ERROR_PLUGIN_VERSION_MISSMATCH);
    dlclose(dlHandle_);
...
int ecmcPluginLib::exeEnterRTFunc() {
  ...
  if (data_->realtimeEnterFnc) {
    ...
    if (errorCode) {
      LOGERR(
        "%s/%s:%d: Error: Plugin %s returned error @ realtimeEnterFnc() (0x%x).\n",
        __FILE__,
        __FUNCTION__,
        __LINE__,
        libFilenameWP_,
...
int ecmcPluginLib::exeExitRTFunc() {
  ...
  if (data_->realtimeExitFnc) {
    ...
    if (errorCode) {
      LOGERR(
        "%s/%s:%d: Error: Plugin %s returned error @ realtimeExitFnc() (0x%x).\n",
        __FILE__,
        __FUNCTION__,
        __LINE__,
        libFilenameWP_,
