...
int ecmcEcSDO::writeComplete(ec_master_t *master,uint16_t slavePosition,uint16_t sdoIndex, uint32_t value, size_t byteSize)
{
  uint32_t abortCode=0;
  if(byteSize>4){
    LOGERR("%s/%s:%d: ERROR: SDO object 0x%x at slave position %d: Write failed, byte size to large (0x%x).\n",__FILE__, __FUNCTION__, __LINE__,sdoIndex,slavePosition,ERROR_EC_SDO_SIZE_TO_LARGE);
    return ERROR_EC_SDO_SIZE_TO_LARGE;
  }
  uint8_t buffer[4];
  memset(&buffer[0],0,4);
  ...
  int errorCode=ecrt_master_sdo_download_complete(master,slavePosition, sdoIndex, &buffer[0], byteSize, &abortCode);
  if(errorCode || abortCode){
    LOGERR("%s/%s:%d: ERROR: SDO object 0x%x at slave position %d: Write failed with sdo error code %d, abort code 0x%x (0x%x).\n",__FILE__, __FUNCTION__, __LINE__,sdoIndex,slavePosition,errorCode,abortCode,ERROR_EC_SDO_WRITE_FAILED);
  }
  return errorCode;
}
