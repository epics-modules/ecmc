...
class Test(unittest.TestCase):
    ...
    per90_dialPosition  = round((1 * m1.DLLM + 9 * m1.DHLM) / 10)
    ...
    print "m1.DLLM=%d m1.DHLM=%d per90_dialPosition=%d" % (m1.DLLM, m1.DHLM, per90_dialPosition)

    def setUp(self):
        print 'set up'
        print 'self.m1.DLLM=%f self.m1.DHLM=%f self.per90_dialPosition=%f' % (self.m1.DLLM, self.m1.DHLM, self.per90_dialPosition)
    ...
    def test_TC_1212(self):
        if (self.msta & self.MSTA_BIT_HOMED):
            tc_no = "TC-1212-30-percent-dialPosition"
            print '%s' % tc_no
            destination = self.per90_dialPosition
            ...
            print '%s postion=%f per90_dialPosition=%f' % (
                tc_no, dialPosition, self.per90_dialPosition)
    ...
    def test_TC_1214(self):
        if (self.msta & self.MSTA_BIT_HOMED):
            tc_no = "TC-1214-30-percent-dialPosition"
            print '%s' % tc_no
            destination = self.per90_dialPosition
            moveDialPosition(self.m1, tc_no, destination,
                             self.moving_velocity, self.acceleration)
